...
export const getAllOrders = async (req, res, next) => {
  try {
    ...
    // [UPDATE] Gọi hàm getAll từ DonHangAdmin
    const orders = await DonHangAdmin.getAll(filters);
...
export const getOrderById = async (req, res, next) => {
  try {
    const { id } = req.params;
    // [UPDATE] Gọi hàm getById từ DonHangAdmin
    const order = await DonHangAdmin.getById(id);
...
export const updateOrderStatus = async (req, res, next) => {
  try {
    ...
    const newStatus = parseInt(tinhTrangDonHang);
    ...
    //  Gọi hàm từ DonHangAdmin
    const affectedRows = await DonHangAdmin.updateOrderStatus(
      id,
      tinhTrangDonHang
    );
    ...
    if (newStatus === 2) {
      await db.query(
        `UPDATE DonHang SET TinhTrangThanhToan = 2 WHERE MaDonHang = ?`,
        [id]
      );
    }
...
export const updatePaymentStatus = async (req, res, next) => {
  try {
    ...
    // Phần này đơn giản nên có thể query trực tiếp hoặc chuyển vào model nếu muốn
...
export const getOrderStats = async (req, res, next) => {
  try {
    // [UPDATE] Gọi hàm từ DonHangAdmin
    const stats = await DonHangAdmin.getOrderStats();
...
export const getRevenueByDate = async (req, res, next) => {
  try {
    ...
    // [UPDATE] Gọi hàm từ DonHangAdmin
    const data = await DonHangAdmin.getRevenueByDate(fromDate, toDate);
...
export const getTopSellingProducts = async (req, res, next) => {
  try {
    ...
    // [UPDATE] Gọi hàm từ DonHangAdmin
    const data = await DonHangAdmin.getTopSellingProducts(limit);
...
export const cancelOrder = async (req, res, next) => {
  ...
  try {
    ...
    try {
      // [UPDATE] Gọi hàm từ DonHangAdmin để cập nhật trạng thái hủy
      await DonHangAdmin.updateOrderStatus(id, 3); // 3 là Đã hủy

      // Logic trả tồn kho giữ nguyên (vẫn gọi BienThe model để cộng lại kho)
